...
public class InterviewDAO extends BaseDAO<InterviewEntity, String> {
    ...
    public List<qEmailCandidateInfoEntity> findQEmailCandidateByApplicationId(String id) {
        List<qEmailCandidateInfoEntity> result = new ArrayList<>();
        StringBuilder sql = new StringBuilder();
        sql.append("select interview.*, application.can_first_name_th ,application.can_last_name_th, application.contact_email ,hr.hr_first_name ,hr.hr_last_name, hr.hr_position ,hr.hr_email ,hr.hr_phone ,position_link_form.position_id , position_link_form.position_name ");
        sql.append("from interview ");
        sql.append("inner join application on application.application_id = interview.application_id ");
        sql.append("inner join position_link_form on position_link_form.position_id  = application.position_id ");
        sql.append("inner join hr on hr.hr_id = interview.hr_id ");
        ...
        Query query = sessionFactory.getCurrentSession().createNativeQuery(sql.toString(), qEmailCandidateInfoEntity.class);
        query.setParameter("id", id);
        result = query.getResultList();
        return result;
    }
